...
class TestGenerateModelCard:
    ...
    def test_generates_card_with_defaults(self, mock_import: MagicMock) -> None:
        ...
        content = generate_model_card(repo_id="velocitylabo/tobira-spam-bert-ja")

        assert "velocitylabo/tobira-spam-bert-ja" in content
    ...
    def test_generates_card_with_metrics(self, mock_import: MagicMock) -> None:
        ...
        content = generate_model_card(
            repo_id="velocitylabo/tobira-spam-bert-ja", metrics=metrics
        )
...
class TestPushToHub:
    ...
    def test_push_uploads_model(self, mock_import: MagicMock, tmp_path: Path) -> None:
        ...
        url = push_to_hub(
            model_dir=model_dir,
            repo_id="velocitylabo/tobira-spam-bert-ja",
            token="test-token",
        )

        assert url == "https://huggingface.co/velocitylabo/tobira-spam-bert-ja"
        mock_api.create_repo.assert_called_once_with(
            repo_id="velocitylabo/tobira-spam-bert-ja", private=False, exist_ok=True
        )
    ...
    def test_push_private_repo(self, mock_import: MagicMock, tmp_path: Path) -> None:
        ...
        mock_api.create_repo.assert_called_once_with(
            repo_id="velocitylabo/tobira-spam-bert-ja", private=True, exist_ok=True
        )
...
class TestHubPushParser:
    def test_parser_has_hub_push_subcommand(self) -> None:
        ...
        args = parser.parse_args([
            "hub-push", "/tmp/model", "--repo-id", "velocitylabo/tobira-spam-bert-ja"
        ])
    ...
    def test_parser_hub_push_defaults(self) -> None:
        ...
        assert args.repo_id == "velocitylabo/tobira-spam-bert-ja"
...
class TestHubPullParser:
    def test_parser_has_hub_pull_subcommand(self) -> None:
        ...
        parser = build_parser()
        args = parser.parse_args([
            "hub-pull", "velocitylabo/tobira-spam-bert-ja", "--local-dir", "/tmp/model"
        ])
    ...
    def test_parser_hub_pull_missing_local_dir_exits(self) -> None:
        ...
        parser = build_parser()
        with pytest.raises(SystemExit):
            parser.parse_args(["hub-pull", "velocitylabo/tobira-spam-bert-ja"])
...
class TestHubPushRun:
    ...
    def test_push_success(
        ...
        mock_push.return_value = "https://huggingface.co/velocitylabo/tobira-spam-bert-ja"
        result = main([
            "hub-push", str(model_dir),
            "--repo-id", "velocitylabo/tobira-spam-bert-ja",
        ])
...
class TestHubPullRun:
    ...
    def test_pull_success(
        ...
        result = main([
            "hub-pull", "velocitylabo/tobira-spam-bert-ja",
